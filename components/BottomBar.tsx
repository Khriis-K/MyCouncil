...
const BottomBar: React.FC<BottomBarProps> = ({ 
  ...
  return (
    <footer 
      className={`absolute bottom-0 left-0 right-0 backdrop-blur-md p-4 z-40 transition-all duration-300 ${isDisabled ? 'opacity-50 pointer-events-none' : 'opacity-100'} ${isHighlighted ? 'ring-4 ring-primary ring-opacity-50 shadow-[0_0_30px_rgba(79,70,229,0.6)]' : ''}`}
      style={{
    ...
    >
      <div className="max-w-screen-2xl mx-auto flex items-center space-x-4">
        ...
        {/* Debate Toggle */}
        <div className="flex items-center space-x-3 px-4" style={{ borderRight: '1px solid var(--border-primary)' }}>
          <label htmlFor="debate-mode" className="text-sm font-medium cursor-pointer select-none" style={{ color: 'var(--text-secondary)' }}>
            Debate Mode
          </label>
          <button
            id="debate-mode"
